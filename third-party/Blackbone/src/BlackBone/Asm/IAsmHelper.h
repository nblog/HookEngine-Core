...
namespace blackbone
{
    ...
    {
    public:
        BLACKBONE_API IAsmHelper( uint32_t arch = asmjit_::kArchHost ) 
        ...
        {
            asmjit_::Label l = _assembler.newLabel();

            _assembler.call( l ); _assembler.bind( l );
            _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::esp, 4 ), 0x23 );
            _assembler.add( asmjit_::host::dword_ptr( asmjit_::host::esp ), 0xD );
        ...
        BLACKBONE_API inline asmjit_::X86Assembler* assembler() { return &_assembler; }
        BLACKBONE_API inline asmjit_::X86Assembler* operator ->() { return &_assembler; }
    ...
    protected:
        asmjit_::JitRuntime _runtime;
        asmjit_::X86Assembler _assembler;
    };
}