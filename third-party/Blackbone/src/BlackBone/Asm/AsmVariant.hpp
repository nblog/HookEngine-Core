...
struct AsmVariant
{
    ...
    BLACKBONE_API AsmVariant( asmjit_::GpReg _reg )
        : type( reg )
        , size( sizeof( uintptr_t ) )
        , reg_val( _reg ) { }

    // Stack variable
    BLACKBONE_API AsmVariant( asmjit_::Mem _mem )
        : type( mem )
        , size( sizeof( uintptr_t ) )
        , mem_val( _mem ) { }
    ...
    BLACKBONE_API AsmVariant( const asmjit_::Mem* _mem )
        : AsmVariant( const_cast<asmjit_::Mem*>(_mem) ) { }
    ...
    asmjit_::GpReg reg_val;  // General purpose register
    asmjit_::Mem   mem_val;  // Memory pointer
...
struct AsmFunctionPtr: public AsmVariant
{
    ...
    AsmFunctionPtr( asmjit_::GpReg reg_ )
        : AsmVariant( reg_ ) { }
};
