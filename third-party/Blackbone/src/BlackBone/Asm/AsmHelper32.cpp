...
void AsmHelper32::GenPrologue( bool switchMode /*= false*/ )
{
    ...
    {
        _assembler.push( asmjit_::host::ebp );
        _assembler.mov( asmjit_::host::ebp, asmjit_::host::esp );
    }
}
...
void AsmHelper32::GenEpilogue( bool switchMode /*= false*/ , int retSize /*= -1 */ )
{
    ...
    {
        _assembler.mov( asmjit_::host::esp, asmjit_::host::ebp );
        _assembler.pop( asmjit_::host::ebp );
    }
...
void AsmHelper32::GenCall( const AsmFunctionPtr& pFN, const std::vector<AsmVariant>& args, eCalligConvention cc /*= CC_stdcall*/ )
{
    ...
    if(pFN.type == AsmVariant::imm)
    {
        ...
        _assembler.mov( asmjit_::host::eax, pFN.imm_val );
        _assembler.call( asmjit_::host::eax );
    }
    // Already in register
    ...
    if (cc == cc_cdecl)
    {
        ...
        _assembler.add( asmjit_::host::esp, argsize );
    }
...
void AsmHelper32::ExitThreadWithStatus( uint64_t pExitThread, uint64_t resultPtr )
{
    if (resultPtr != 0)
    {
        _assembler.mov( asmjit_::host::edx, resultPtr );
        _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::edx ), asmjit_::host::eax );
    }

    // NtTerminateThread( NULL, eax );
    _assembler.push( asmjit_::host::eax );
    _assembler.push( 0 );
    _assembler.mov( asmjit_::host::eax, pExitThread );
    _assembler.call( asmjit_::host::eax );
...
{
    _assembler.mov( asmjit_::host::ecx, ResultPtr );

    // Return 64bit value
    if (rtype == rt_int64)
    {
        _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::ecx, 4 ), asmjit_::host::eax );
        _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::ecx ), asmjit_::host::edx );
    }
    else if (rtype == rt_int32)
        _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::ecx ), asmjit_::host::eax );

    // Save last NT status
    _assembler.mov( asmjit_::host::edx, asmjit_::host::dword_ptr_abs( 0x18 ).setSegment( asmjit_::host::fs ) );
    _assembler.add( asmjit_::host::edx, 0x598 + 0x197 * sizeof( uint32_t ) );
    _assembler.mov( asmjit_::host::edx, asmjit_::host::dword_ptr( asmjit_::host::edx ) );
    _assembler.mov( asmjit_::host::eax, errPtr );
    _assembler.mov( asmjit_::host::dword_ptr( asmjit_::host::eax ), asmjit_::host::edx );

    // SetEvent(hEvent)
    // NtSetEvent(hEvent, NULL)
    _assembler.push( 0 );
    _assembler.mov( asmjit_::host::eax, EventPtr );
    _assembler.mov( asmjit_::host::eax, asmjit_::host::dword_ptr( asmjit_::host::eax ) );
    _assembler.push( asmjit_::host::eax );
    _assembler.mov( asmjit_::host::eax, pSetEvent );
    _assembler.call( asmjit_::host::eax );
}
...
void AsmHelper32::PushArg( const AsmVariant& arg, eArgType regidx /*= AT_stack*/ )
{
    ...
    case AsmVariant::dataStruct:
        {
            ...
            _assembler.sub( asmjit_::host::esp, realSize );
            _assembler.mov( asmjit_::host::esi, arg.new_imm_val );
            _assembler.mov( asmjit_::host::edi, asmjit_::host::esp);

            // Preserve ecx value, may be __thiscall or __fastcall function
            _assembler.mov( asmjit_::host::eax, asmjit_::host::ecx );

            _assembler.mov( asmjit_::host::ecx, arg.size );
            _assembler.rep_movsb();

            // Restore registers
            _assembler.mov( asmjit_::host::ecx, asmjit_::host::eax );
        }
        break;
    ...
    case AsmVariant::mem_ptr:        
        _assembler.lea( asmjit_::host::eax, arg.mem_val );
        PushArgp( asmjit_::host::eax, regidx );
        break;
...
{
    static const asmjit_::GpReg regs[] = { asmjit_::host::ecx, asmjit_::host::edx };
