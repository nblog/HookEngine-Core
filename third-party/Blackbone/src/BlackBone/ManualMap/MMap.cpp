...
call_result_t<uint64_t> MMap::RunModuleInitializers( ImageContextPtr pImage, DWORD dwReason, CustomArgs_t* pCustomArgs /*= nullptr*/ )
{
    ...
    if (_pAContext.valid() && pActivateActx)
    {
        ...
        (*a)->mov( (*a)->zax, asmjit_::host::dword_ptr( (*a)->zax ) );
...
{   
    ...
    if (switchMode)
    {
        ...
        (*a)->mov( asmjit_::host::eax, static_cast<uint32_t>(pCreateActx->procAddress) );
        (*a)->call( (*a)->zax );
        (*a)->mov( asmjit_::host::edx, _pAContext.ptr<uint32_t>() );
        ...
        (*a)->push( (*a)->zax );
        (*a)->push( uint32_t( 0 ) );
        (*a)->mov( asmjit_::host::eax, static_cast<uint32_t>(pTermThd->procAddress) );
        (*a)->call( (*a)->zax );
        (*a)->ret( 4 );
