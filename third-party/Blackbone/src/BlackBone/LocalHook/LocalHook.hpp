...
private:
    ...
    bool HookInline()
    {
        ...
        //
#ifdef USE64
        (*jmpToThunk)->mov( asmjit_::host::rax, (uint64_t)this->_buf );
        (*jmpToThunk)->jmp( asmjit_::host::rax );
...
#ifdef USE64
        // mov gs:[0x28], this
        (*jmpToHook)->mov( asmjit_::host::rax, (uint64_t)this );
        (*jmpToHook)->mov( asmjit_::host::qword_ptr_abs( 0x28 ).setSegment( asmjit_::host::gs ), asmjit_::host::rax );
#else
        // mov fs:[0x14], this
        (*jmpToHook)->mov( asmjit::host::dword_ptr_abs( 0x14 ).setSegment( asmjit::host::fs ) , (uint32_t)this );
#endif // USE64

        (*jmpToHook)->jmp( (asmjit_::Ptr)&HookHandler<Fn, C>::Handler );
