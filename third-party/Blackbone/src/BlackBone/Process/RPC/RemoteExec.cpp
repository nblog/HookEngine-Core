    ...
    )
{
    ...
    if(switchMode)
    {
        ...
        if (createActStack)
        {
            ...
            (*a)->mov( (*a)->zdx, asmjit_::host::dword_ptr_abs( 0x30 ).setSegment( asmjit_::host::gs ) );
    ...
    (*a)->mov( asmjit_::host::dword_ptr( (*a)->zdx ), (*a)->zax );
...
NTSTATUS RemoteExec::ExecInAnyThread( PVOID pCode, size_t size, uint64_t& callResult, ThreadPtr& thd )
{
    ...
    {
        const int count = 15;
        static const asmjit_::GpReg regs[] =
        {
            asmjit_::host::rax, asmjit_::host::rbx, asmjit_::host::rcx, asmjit_::host::rdx, asmjit_::host::rsi,
            asmjit_::host::rdi, asmjit_::host::r8,  asmjit_::host::r9,  asmjit_::host::r10, asmjit_::host::r11,
            asmjit_::host::r12, asmjit_::host::r13, asmjit_::host::r14, asmjit_::host::r15, asmjit_::host::rbp
        };
        ...
        //
        (*a)->sub( asmjit_::host::rsp, count * sizeof( uint64_t ) );
        (*a)->pushf(); 

        // Save registers
        for (int i = 0; i < count; i++)
            (*a)->mov( asmjit_::Mem( asmjit_::host::rsp, i * sizeof( uint64_t ) ), regs[i] );
        ...
        // Restore registers
        for (int i = 0; i < count; i++)
            (*a)->mov( regs[i], asmjit_::Mem( asmjit_::host::rsp, i * sizeof( uint64_t ) ) );

        (*a)->popf();
        (*a)->add( asmjit_::host::rsp, count * sizeof( uint64_t ) );
    ...
    else
    {
        ...
        a->GenCall( _userCode.ptr(), { _userData.ptr() } );
        (*a)->add( asmjit_::host::esp, sizeof( uint32_t ) );
...
{
    auto a = AsmFactory::GetAssembler( _process.core().isWow64() );
    asmjit_::Label l_loop = (*a)->newLabel();
    ...
    )
{
    ...
    for (auto& arg : args)
    {
        ...
        if (arg.type == AsmVariant::imm && arg.size > sizeof( uint32_t ) && a.assembler()->getArch() == asmjit_::kArchX86)
        {
    ...
    if (retType == rt_float || retType == rt_double)
    {
        ...
        if (retType == rt_double)
            a->movsd( asmjit_::Mem( a->zax, 0 ), asmjit_::host::xmm0 );
        else
            a->movss( asmjit_::Mem( a->zax, 0 ), asmjit_::host::xmm0 );
#else
