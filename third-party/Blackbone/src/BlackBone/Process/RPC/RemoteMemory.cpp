...
void RemoteMemory::BuildGenericHookFn( OperationType opType )
{
    ...
    asmjit_::Label skip1 = a->newLabel();
    ...
    a->sub( asmjit_::host::rsp, stack_size );

    // Call original function
    for (int i = 0; i < argc[opType] - 4; i++)
    {
        a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + 0x20 + i * 8 ) );
        a->mov( asmjit_::host::qword_ptr( asmjit_::host::rsp, 0x20 + i * 8 ), asmjit_::host::rax );
    }

    a->mov( asmjit_::host::rax, _targetShare + hookDataOfs + FIELD_OFFSET( HookData, original_code ) );
    a->call( asmjit_::host::rax );

    // Test if call was successful
    a->test( asmjit_::host::rax, asmjit_::host::rax );
    a->jnz( skip1 );

    // Test if memory was committed
    if (opType == MemVirtualAlloc)
    {
        a->mov( asmjit_::host::eax, asmjit_::host::dword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + 0x20 ) );
        a->cmp( asmjit_::host::eax, MEM_COMMIT );
        a->jne( skip1 );
    }
    ...
    // Storage pointer
    a->lea( asmjit_::host::rdx, data );

    // Allocation address
    if (allocIdx[opType] > 3)
        a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + 0x20 + (allocIdx[opType] - 4) * 8 ) );
    else
        a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + allocIdx[opType] * 8 ) );

    if (opType != MemUnmapSection)
        a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rax ) );

    a->mov( asmjit_::host::qword_ptr( asmjit_::host::rdx, FIELD_OFFSET( OperationData, allocAddress ) ), asmjit_::host::rax );

    if (sizeIdx[opType] != -1)
    {
        // Region size
        if (sizeIdx[opType] > 3)
            a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + 0x20 + (sizeIdx[opType] - 4) * 8 ) );
        else
            a->mov( asmjit_::host::rax, asmjit_::host::qword_ptr( asmjit_::host::rsp, (int32_t)stack_size + 0x8 + sizeIdx[opType] * 8 ) );

        a->mov( asmjit_::host::eax, asmjit_::host::dword_ptr( asmjit_::host::rax ) );
        a->mov( asmjit_::host::dword_ptr( asmjit_::host::rdx, FIELD_OFFSET( OperationData, allocSize ) ), asmjit_::host::eax );
    }
    else
        a->mov( asmjit_::host::dword_ptr( asmjit_::host::rdx, FIELD_OFFSET( OperationData, allocSize ) ), 0 );

    // Operation type
    a->mov( asmjit_::host::dword_ptr( asmjit_::host::rdx, FIELD_OFFSET( OperationData, allocType ) ), opType );
    ...
    // Ignore return value
    a->xor_( asmjit_::host::rax, asmjit_::host::rax );

    a->bind( skip1 );
    a->add( asmjit_::host::rsp, stack_size );
    a->ret();
